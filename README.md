...
| Story ID | Título | Descripción | Criterios de Aceptación | Relacionado con (Epic ID) |
| :------- | :----- | :---------- | :---------------------- | :------------------------ |
| **TS01** | Configuración base del proyecto | Como equipo de desarrollo, queremos configurar la estructura inicial del proyecto para asegurar una base ordenada y mantenible. | **Escenario 1 - Estructura creada:**<br>Dado que el equipo inicia el desarrollo del sistema.<br>Cuando se crea la estructura base del proyecto.<br>Entonces los módulos principales quedan organizados correctamente.<br><br>**Escenario 2 - Ejecución correcta:**<br>Dado que el proyecto base fue configurado.<br>Cuando el equipo ejecuta la aplicación localmente.<br>Entonces el sistema debe iniciar sin errores críticos. | EP01 |
| **TS02** | Integración del IoT Edge Gateway | Como equipo de desarrollo, queremos implementar la comunicación entre los dispositivos IoT y el IoT Edge Gateway para centralizar los eventos del refugio. | **Escenario 1 - Recepción de datos IoT:**<br>Dado que un dispositivo IoT envía información al sistema.<br>Cuando el IoT Edge Gateway recibe los datos.<br>Entonces el evento deberá registrarse correctamente.<br><br>**Escenario 2 - Error de comunicación:**<br>Dado que un dispositivo no puede comunicarse con el IoT Edge Gateway.<br>Cuando ocurre una falla de conexión.<br>Entonces el sistema deberá marcar el evento como pendiente o no recibido. | EP01 |
| **TS03** | Modelo de datos para seguimiento veterinario | Como equipo de desarrollo, queremos implementar el modelo de datos veterinario para almacenar dietas, observaciones, recomendaciones y reportes de bienestar. | **Escenario 1 - Modelo creado:**<br>Dado que el sistema requiere información veterinaria.<br>Cuando se crean las estructuras de datos correspondientes.<br>Entonces deberán existir registros para dietas, observaciones, recomendaciones y reportes.<br><br>**Escenario 2 - Registro correcto:**<br>Dado que el modelo de datos está disponible.<br>Cuando el veterinario registra una observación o recomendación.<br>Entonces la información deberá almacenarse correctamente. | EP02 |
| **TS04** | Configuración de notificaciones push | Como equipo de desarrollo, queremos configurar el servicio de notificaciones push para enviar alertas a los usuarios ante eventos relevantes. | **Escenario 1 - Envío de notificación:**<br>Dado que el sistema genera una alerta importante.<br>Cuando el backend solicita el envío de una notificación push.<br>Entonces el usuario deberá recibir la notificación en su dispositivo.<br><br>**Escenario 2 - Fallo en el envío:**<br>Dado que el servicio de notificaciones no responde.<br>Cuando el sistema intenta enviar una alerta push.<br>Entonces deberá registrar el fallo para su revisión. | EP01 |
| **TS05** | Desarrollo de reportes veterinarios | Como equipo de desarrollo, queremos implementar la base técnica para generar reportes veterinarios de bienestar animal. | **Escenario 1 - Generación de reporte:**<br>Dado que existen registros de alimentación, ambiente y alertas.<br>Cuando el sistema procesa la información del animal.<br>Entonces deberá generar un reporte con los eventos relevantes.<br><br>**Escenario 2 - Datos insuficientes:**<br>Dado que el animal tiene pocos registros disponibles.<br>Cuando se solicita el reporte.<br>Entonces el sistema deberá mostrar solo la información existente. | EP02 |
